class PaintAgent {
  ...
  signalPaintBufferChange() {
    var sig = [];
    sig[0] = 'PAINTBUFFER';
    sig[1] = 'CHANGE';
		sig.data = new Object();
    sig.data.buffer = new Object();
    sig.data.buffer.canvas = this.state.paintCanvas;
    sig.data.buffer.context = this.state.paintContext;
    ...
    if (sig[0] == 'APPMOUSE') {
      ...
      switch (sig[1]) {
        ...
        case 'ZOOMRELATIVE':
          ...
          this.state.drawFunction.setFunctionData({
        ...
        case 'EXPORTIMAGE':
          ...
          break;
        case 'EXPORTOTHER':
          var coordOptions = sig.data.coordinates;
					var isOpaqueFunction = sig.data.isOpaqueFunction;
          var getColorIndexFunction = sig.data.getColorIndexFunction;
          var mode = sig.data.colormode;
          var exportdatapresentation = sig.data.exportdatapresentation;
          var exportdatatype = sig.data.exportdatatype;
          var palette = undefined;
          var datastart = sig.data.datastart;
          var datastep = sig.data.datastep;

          if ( mode == '4bpp' || mode == '8bpp' ) {
            palette = sig.data.colorPalette;
          }
          ...
          sig.data.options = {
            mode: mode, //TODO 'sprite.16color'
						coordinates: coordOptions,
						isOpaqueFunction: isOpaqueFunction,
            getColorIndexFunction: getColorIndexFunction,
            colorPalette: palette,
            exportdatapresentation: exportdatapresentation,
